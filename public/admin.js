...
function applyPosterTheme(themeId) {
  if (!posterEl) return;
  posterEl.classList.remove('theme-neon', 'theme-dark', 'theme-sunset', 'theme-nuit', 'theme-luxe', 'theme-pastel');
...
if (savePrizesBtn) {
  savePrizesBtn.addEventListener('click', async () => {
    ...
    const hasOver = cleanedForTotal.some((p) => Number(p.probability || 0) > 100 || Number(p.probability || 0) < 0);
